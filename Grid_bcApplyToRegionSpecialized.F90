...
#include "Flash.h"
  ...
  real :: rhoZone, presZone, enerZone, ekinZone, tempZone, velxZone, velyZone, velzZone
...
!			dt = dr_simTime
              ...
              rhoZone  = sim_rho_j !+ (sim_rho_j-sim_rho_a)/cosh((yy/sim_rj)**sim_mjb)
              presZone = sim_p_a !+ (sim_p_j-sim_p_a)/cosh((yy/sim_rj)**sim_mjb)

              velxZone = sim_jet_vel!*(1.0+0.33*sin(2.0*3.1416*dt/0.71)) !/cosh((yy/sim_rj)**sim_mjb)
              velyZone = 0.0
              velzZone = 0.0

              else

              rhoZone  = sim_rho_a !- (sim_rho_j-sim_rho_a)/cosh((yy/sim_rj)**sim_mjb)
              presZone = sim_p_a !- (sim_p_j-sim_p_a)/cosh((yy/sim_rj)**sim_mjb)

              velxZone = 0.0 !sim_jet_vel!/cosh((yy/sim_rj)**sim_mjb)
              velyZone = 0.0
              velzZone = 0.0
              ...
              regionData(i1,i2,i3,VELX_VAR)=velxZone
              regionData(i1,i2,i3,VELY_VAR)=velyZone
              regionData(i1,i2,i3,VELZ_VAR)=velzZone
